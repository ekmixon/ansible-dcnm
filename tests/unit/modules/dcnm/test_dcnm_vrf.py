...
class TestDcnmVrfModule(TestDcnmModule):
    ...
    playbook_vrf_lite_update_vlan_config = test_data.get('playbook_vrf_lite_update_vlan_config')
    ...
    def setUp(self):
        ...
        self.mock_dcnm_ip_sn = patch('ansible_collections.cisco.dcnm.plugins.modules.dcnm_vrf.get_fabric_inventory_details')
    ...
    def load_fixtures(self, response=None, device=''):
        ...
        if 'get_have_failure' in self._testMethodName:
            ...
            self.run_dcnm_send.side_effect = [self.blank_data, self.blank_data, self.error1,
                                              self.blank_data]

        elif 'error2' in self._testMethodName:
            self.run_dcnm_send.side_effect = [self.blank_data, self.blank_data, self.error2, self.blank_data]

        elif 'error3' in self._testMethodName:
            self.run_dcnm_send.side_effect = [self.blank_data, self.blank_data, self.attach_success_resp, self.error3,
                                              self.blank_data]
            ...
            self.run_dcnm_send.side_effect = [self.mock_vrf_object_dcnm_only, self.mock_vrf_attach_object_dcnm_only,
                                              self.mock_vrf_attach_get_ext_object_dcnm_att1_only, self.mock_vrf_attach_get_ext_object_dcnm_att2_only,
                                              self.attach_success_resp, self.deploy_success_resp,
                                              obj1,
                                              obj2,
                                              self.delete_success_resp]
            ...
            self.run_dcnm_send.side_effect = [self.mock_vrf_object, self.mock_vrf_attach_object,
                                              ...
                                              self.mock_vrf_attach_object_query, self.mock_vrf_attach_get_ext_object_merge_att1_only,
                                              self.mock_vrf_attach_get_ext_object_merge_att2_only]
    ...
    def test_dcnm_vrf_merged_lite_redeploy(self):
        set_module_args(dict(state='merged',
                             fabric='test_fabric', config=self.playbook_vrf_lite_config))
        result = self.execute_module(changed=True, failed=False)
        self.assertEqual(result.get('diff')[0]['vrf_name'], 'test_vrf_1')
    ...
    def test_dcnm_vrf_merged_lite_new(self):
        set_module_args(dict(state='merged', fabric='test_fabric', config=self.playbook_vrf_lite_config))
        result = self.execute_module(changed=True, failed=False)
        self.assertTrue(result.get('diff')[0]['attach'][0]['deploy'])
        self.assertTrue(result.get('diff')[0]['attach'][1]['deploy'])
        self.assertEqual(result.get('diff')[0]['attach'][0]['ip_address'], '10.10.10.224')
    ...
    def test_dcnm_vrf_merged_lite_invalidrole(self):
        set_module_args(dict(state='merged', fabric='test_fabric', config=self.playbook_vrf_lite_inv_config))
        result = self.execute_module(changed=False, failed=True)
        self.assertEqual(result['msg'], 'VRF LITE cannot be attached to switch 10.10.10.225 with role leaf')
    ...
    def test_dcnm_vrf_merged_lite_update(self):
        set_module_args(dict(state='merged', fabric='test_fabric', config=self.playbook_vrf_lite_update_config))
        result = self.execute_module(changed=True, failed=False)
        self.assertTrue(result.get('diff')[0]['attach'][0]['deploy'])
    ...
    def test_dcnm_vrf_error2(self):
        set_module_args(dict(state='merged', fabric='test_fabric', config=self.playbook_config))
        result = self.execute_module(changed=False, failed=True)
        self.assertIn("Entered VRF VLAN ID 203 is in use already",str(result['msg']['DATA'].values()))
    ...
    def test_dcnm_vrf_replace_lite_changes(self):
        set_module_args(dict(state='replaced', fabric='test_fabric', config=self.playbook_vrf_lite_replace_config))
        result = self.execute_module(changed=True, failed=False)
        self.assertTrue(result.get('diff')[0]['attach'][0]['deploy'])
        self.assertFalse(result.get('diff')[0]['attach'][1]['deploy'])
    ...
    def test_dcnm_vrf_replace_lite_without_changes(self):
        set_module_args(dict(state='replaced', fabric='test_fabric', config=self.playbook_vrf_lite_config))
        result = self.execute_module(changed=False, failed=False)
        self.assertFalse(result.get('diff'))
        self.assertFalse(result.get('response'))
    ...
    def test_dcnm_vrf_lite_override_with_additions(self):
        set_module_args(dict(state='overridden', fabric='test_fabric', config=self.playbook_vrf_lite_config))
        result = self.execute_module(changed=True, failed=False)
        self.assertTrue(result.get('diff')[0]['attach'][0]['deploy'])
        self.assertTrue(result.get('diff')[0]['attach'][1]['deploy'])
        self.assertEqual(result.get('diff')[0]['attach'][0]['ip_address'], '10.10.10.224')
    ...
    def test_dcnm_vrf_override_with_deletions(self):
        ...
        self.assertEqual(result['response'][1]['DATA']['status'], '')
        self.assertEqual(result['response'][1]['RETURN_CODE'], self.SUCCESS_RETURN_CODE)
        self.assertEqual(result['response'][4]['DATA']['test-vrf-2--XYZKSJHSMK2(leaf2)'], 'SUCCESS')
        self.assertEqual(result['response'][4]['DATA']['test-vrf-2--XYZKSJHSMK3(leaf3)'], 'SUCCESS')

    def test_dcnm_vrf_lite_override_with_deletions(self):
        set_module_args(dict(state='overridden', fabric='test_fabric', config=self.playbook_vrf_lite_replace_config))
        result = self.execute_module(changed=True, failed=False)
        self.assertTrue(result.get('diff')[0]['attach'][0]['deploy'])
        self.assertFalse(result.get('diff')[0]['attach'][1]['deploy'])
        self.assertEqual(result.get('diff')[0]['attach'][0]['vlan_id'], 202)
    ...
    def test_dcnm_vrf_delete_std_lite(self):
        set_module_args(dict(state='deleted', fabric='test_fabric', config=self.playbook_vrf_lite_config))
        result = self.execute_module(changed=True, failed=False)
        self.assertFalse(result.get('diff')[0]['attach'][0]['deploy'])
        self.assertFalse(result.get('diff')[0]['attach'][1]['deploy'])
        self.assertEqual(result.get('diff')[0]['attach'][0]['vlan_id'], '202')
    ...
    def test_dcnm_vrf_delete_failure(self):
        set_module_args(dict(state='deleted', fabric='test_fabric', config=self.playbook_config))
        result = self.execute_module(changed=False, failed=True)
        self.assertEqual(result['msg']['response'][2], 'Deletion of vrfs test_vrf_1 has failed')
    ...
    def test_dcnm_vrf_query(self):
        ...
        self.assertEqual(result.get('response')[0]['attach'][0]['switchDetailsList'][0]['lanAttachedState'], 'DEPLOYED')
        self.assertEqual(result.get('response')[0]['attach'][0]['switchDetailsList'][0]['vlan'], '202')
        self.assertEqual(result.get('response')[0]['attach'][1]['switchDetailsList'][0]['lanAttachedState'], 'DEPLOYED')
        self.assertEqual(result.get('response')[0]['attach'][1]['switchDetailsList'][0]['vlan'], '202')

    def test_dcnm_vrf_query_vrf_lite(self):
        set_module_args(dict(state='query', fabric='test_fabric', config=self.playbook_vrf_lite_config))
        result = self.execute_module(changed=False, failed=False)
        self.assertFalse(result.get('diff'))
        self.assertEqual(result.get('response')[0]['parent']['vrfName'], 'test_vrf_1')
        self.assertEqual(result.get('response')[0]['parent']['vrfId'], 9008011)
        self.assertEqual(result.get('response')[0]['attach'][0]['switchDetailsList'][0]['lanAttachedState'], 'DEPLOYED')
        self.assertEqual(result.get('response')[0]['attach'][0]['switchDetailsList'][0]['vlan'], '202')
        self.assertEqual(result.get('response')[0]['attach'][0]['switchDetailsList'][0]['extensionValues'], '')
        self.assertEqual(result.get('response')[0]['attach'][1]['switchDetailsList'][0]['lanAttachedState'], 'DEPLOYED')
        self.assertEqual(result.get('response')[0]['attach'][1]['switchDetailsList'][0]['vlan'], '202')
        self.assertEqual(result.get('response')[0]['attach'][1]['switchDetailsList'][0]['extensionValues'], '')
    ...
    def test_dcnm_vrf_validation(self):
        ...
        result = self.execute_module(changed=False, failed=True)
        self.assertEqual(result['msg'], 'ip_address is mandatory under attach parameters')
