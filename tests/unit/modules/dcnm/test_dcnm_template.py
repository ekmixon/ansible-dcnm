...
#################################### FIXTURES ############################
    ...
    def load_template_fixtures (self):
        ...
        if ('_template_merged_in_use' in self._testMethodName):
            ...
            self.run_dcnm_send.side_effect = [
                                              inuse_template1, inuse_template2,
                                              self.validate_resp, self.validate_resp,
                                              self.create_inuse_resp, self.create_inuse_resp]
        ...
        if ('_template_delete_inuse' in self._testMethodName):
            ...
            self.run_dcnm_send.side_effect = [
                                              ...
                                              self.delete_inuse_resp, switches, policies]
        ...
        if ('_template_query_existing_inuse' in self._testMethodName):
            ...
            self.run_dcnm_send.side_effect = [
                                              template1, template2, switches, policies
                                              ]
        ...
        if ('_template_validation_fail' in self._testMethodName):
            ...
            self.run_dcnm_send.side_effect = [
                                              template1, self.validate_fail_resp
                                              ]
...
#################################### TEST-CASES ############################
    ...
    def test_dcnm_template_delete_non_existing(self):
        ...
        # load required config data
        self.playbook_config       = self.config_data.get('template_delete_non_existing_config')
        self.delete_non_exist_resp = self.payloads_data.get('template_delete_no_exist_resp')
    ...
    def test_dcnm_template_replace_one_existing(self):
        ...
        self.assertEqual (('internal policy 101 after replacement' in r[0]['content']), True)
    ...
    def test_dcnm_template_replace_no_description(self):
        ...
        self.assertEqual (('internal policy 101 being replaced' in r[0]['content']), True)
    ...
    def test_dcnm_template_query_existing_inuse(self):
        ...
        self.assertEqual((result['template-policy-map']['template_110_inuse']['POLICY-35967']['fabricName'] in ['test-fabric']), True)
        self.assertEqual((result['template-policy-map']['template_110_inuse']['POLICY-35967']['serialNumber'] in ['SAL1812NTBP', 'FOX1821H035']), True)

        self.assertEqual((result['template-policy-map']['template_111_inuse']['POLICY-46328']['fabricName'] in ['test-fabric']), True)
        self.assertEqual((result['template-policy-map']['template_111_inuse']['POLICY-46328']['serialNumber'] in ['SAL1812NTBP', 'FOX1821H035']), True)
    ...
    def test_dcnm_template_validation_fail(self):
        ...
        self.config_data        = loadPlaybookData('dcnm_template_configs')
        self.payloads_data      = loadPlaybookData('dcnm_template_payloads')
        self.validate_fail_resp = self.payloads_data.get('template_validation_fail_resp')
